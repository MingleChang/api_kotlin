...
interface AppRespository : JpaRepository<App, String> {
    @Query("SELECT a FROM App a WHERE a.createdDate IN(SELECT MAX(createdDate) FROM App GROUP BY packageId)")
    fun findGroupPackageId() : List<App>;

    @Query("SELECT a FROM App a WHERE a.createdDate IN(SELECT MAX(createdDate) FROM App WHERE packageId=:packageId GROUP BY platform)")
    fun findGroupPlatformByPackageId(packageId: String): List<App>;

    @Query("SELECT a FROM App a WHERE a.createdDate IN(SELECT MAX(createdDate) FROM App WHERE packageId=:packageId AND platform=:platform GROUP BY environment)")
    fun findGroupEnvironmentByPackageIdAndPlatform(packageId: String, platform: String): List<App>;

    @Query("SELECT a FROM App a WHERE a.createdDate IN(SELECT MAX(createdDate) FROM App WHERE packageId=:packageId AND platform=:platform AND environment=:environment GROUP BY version)")
